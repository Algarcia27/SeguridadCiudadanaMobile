...
import { enviarEmergencia, subirFotoSupabase } from '@/src/supabaseServices';
...
export default function ReportEmergencyScreen() {
  ...
  const [cuadranteAsignado, setCuadranteAsignado] = useState('');
  ...
  const handleSubmit = async () => {
    if (!tipoEmergencia || !municipio || !ubicacionGps.trim() || !cuadranteAsignado.trim()) {
    ...
    try {
      // Subimos solo la primera imagen si el usuario agregó fotos.
      ...
      setSuccess(true);
      Alert.alert('Reporte enviado', 'Tu reporte de emergencia ha sido registrado.');
      setTimeout(() => router.back(), 1200);
    } catch (err) {
      console.error('handleSubmit emergencia error:', err);
      ...
      >
        <View style={[styles.header, { backgroundColor: colors.surfaceContainer }]}> 
          ...
          <View style={styles.headerText}> 
            <Text style={[styles.title, { color: colors.foreground }]}>Reporte de emergencia</Text>
            <Text style={[styles.subtitle, { color: colors.mutedForeground }]}>Escoge el tipo de emergencia y completa el formulario.</Text>
          </View>
        </View>
        ...
        <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}> 
          <Text style={[styles.sectionTitle, { color: colors.foreground }]}>Tipo de emergencia</Text>
            ...
            {EMERGENCY_TYPES.map((type) => (
              <TouchableOpacity
                ...
                style={[
                  styles.typeButton,
                  {
                    backgroundColor: tipoEmergencia === type ? colors.primary : colors.surfaceContainer,
                    borderColor: colors.border,
                  },
                ]}
                onPress={() => setTipoEmergencia(type)}
              >
                <Text style={[styles.typeButtonText, { color: tipoEmergencia === type ? '#fff' : colors.foreground }]}>{type}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>
        ...
        <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}> 
          <Text style={[styles.sectionTitle, { color: colors.foreground }]}>Dirección / ubicación GPS</Text>
          <TextInput
            ...
            placeholder="Ej. Calle 1, sector Centro o 7.123,-72.123"
            placeholderTextColor={colors.mutedForeground + '88'}
          />
        </View>

        <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}> 
          <Text style={[styles.sectionTitle, { color: colors.foreground }]}>Cuadrante asignado</Text>
          <TextInput
            style={[styles.inputField, { backgroundColor: colors.surfaceContainer, color: colors.foreground, borderColor: colors.border }]}
            value={cuadranteAsignado}
            onChangeText={setCuadranteAsignado}
            placeholder="Ej. Cuadrante 3"
            placeholderTextColor={colors.mutedForeground + '88'}
          />
        </View>
        ...
        <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}> 
          ...
          {attachments.length > 0 ? (
            ...
            <Text style={[styles.hintText, { color: colors.mutedForeground }]}>Puedes agregar evidencia visual para acelerar la atención.</Text>
          )}
        </View>
